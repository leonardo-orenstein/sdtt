...
@author: leo
"""
from math import sin, cos, pi, sqrt, atan, asin, tan
...
#        print()
    ...
    def lqrTracker(self, x, y, orientation,
                           phi, v, length, planner,
                           x_road, y_road, angle_road, v_d,
                           delta_t):
        '''
        An lqr based controller to keep the vehicle on it's track. Based on
        F. Lin, Z. Lin and X. Qiu, "LQR controller for car-like robot,"
        2016 35th Chinese Control Conference (CCC), Chengdu, 2016, pp. 2515-2518.
        '''
        theta_e = angle_road - orientation

        d = (x_road - x)*sin(angle_road) - (y_road - y)*cos(angle_road)
#        d = abs(tan(angle_road)*(x - x_road) + y - y_road)/sqrt(tan(angle_road)**2 + 1)
        X = np.matrix([[d, theta_e]]).T
        dX = np.matrix([[-v_d*sin(theta_e),
                       v_d*tan(phi)/length]])

        A = np.matrix([[0, -v_d], [0, 0]])
        B = np.matrix([[0, v_d/length]]).T

        Q = np.matrix([[10, 0],[0, 5]])
        R = np.matrix([[50]])
